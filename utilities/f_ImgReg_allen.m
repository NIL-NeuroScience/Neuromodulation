...
if ~type
    tmpMasks = parcellationStruct;
    tmpMasks(isnan(parcellationStruct)) = 0;
    hem = [];
    for i = 1 : 12
    ...
    if hem == 2
        ...
        parcellationStruct = struct;
        parcellationStruct.Masks = zeros([size(tmpMasks),2]);

        parcellationStruct.Masks(:,1:W,:,1) = tmpMasks(:,1:W,:);
        parcellationStruct.Masks(:,W:end,:,2) = tmpMasks(:,W:end,:);
    else
        ...
        parcellationStruct = struct;
        parcellationStruct.Masks = zeros([size(tmpMasks),2]);
        parcellationStruct.Masks(:,:,:,side+1) = tmpMasks;
    end
end
...
if type
    ...
    hem = [];
    for i = 1 : 12
        hem(i) = numel(bwboundaries(parcellationStruct(:,:,i)));
    end
    hem = mode(hem);
...
if type
    ...
    if hem == 1
        Vis2 = parcellationStruct(:,:,12);
        SSp_ll2 = parcellationStruct(:,:,5);
        points.p2.point1 = round(findTop(Vis2));
        points.p2.point2 = round(findTop(SSp_ll2));
