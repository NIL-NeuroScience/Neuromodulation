...
tmp = struct;
tmp.LR_perf = f_regImages(Fig2.LR_perf,refParcellation,settings.allen_masks,0).*refBM;
tmp.IRFx2_perf = f_regImages(Fig2.IRFx2_perf,refParcellation,settings.allen_masks,0).*refBM;
tmp.s_LR_perf = f_regImages(shuffled.LR_perf,refParcellation,settings.allen_masks,0).*refBM;
tmp.s_IRFx2_perf = f_regImages(shuffled.IRFx2_perf,refParcellation,settings.allen_masks,0).*refBM;
tmp.g_LR_perf = f_regImages(Fig2.LR_perf,refParcellation,settings.allen_masks,0).*refBM;
tmp.g_IRFx2_perf = f_regImages(Fig2.IRFx2_perf,refParcellation,settings.allen_masks,0).*refBM;
tmp.LR_A = f_regImages(Fig2.LR_A,refParcellation,settings.allen_masks,0).*refBM;
tmp.LR_B = f_regImages(Fig2.LR_B,refParcellation,settings.allen_masks,0).*refBM;
tmp.IRFx2_A = f_regImages(Fig2.IRFx2_A,refParcellation,settings.allen_masks,0).*refBM;
tmp.IRFx2_B = f_regImages(Fig2.IRFx2_B,refParcellation,settings.allen_masks,0).*refBM;
...
for i = 1:numel(NE_order)
    ...
    subAvg.Fig2.IRFx2_IRF(:,:,i) = mean(cat(3,Fig2.IRFx2_IRF{NE_order(i).Runs}),3);
end
...
runData = load('sub-Thy1-301_ses-24-08-01_run-03_irun-01_behavior+ophys.mat');
...
colorbar off;
title '';
exportgraphics(f, fullfile(fig_savePath,'Figure2_B2.jpg'),'Resolution',300,'BackgroundColor',[1 1 1]);
...
colorbar off;
title '';
exportgraphics(f, fullfile(fig_savePath,'Figure2_D.jpg'),'Resolution',300,'BackgroundColor',[1 1 1]);
...
colorbar off;
title '';
exportgraphics(f, fullfile(fig_savePath,'Figure2_E.jpg'),'Resolution',300,'BackgroundColor',[1 1 1]);
...
colorbar off;
title '';
exportgraphics(f, fullfile(fig_savePath,'Figure2_F2.jpg'),'Resolution',300,'BackgroundColor',[1 1 1]);
...
error1 = std(subAvg.Fig2.IRFx2_IRF(:,1,:),0,3)/sqrt(numel(NE_order));
f_plotLineError(-5:0.1:10,meanSig1,error1,color=c_Ca);
meanSig2 = mean(subAvg.Fig2.IRFx2_IRF(:,2,:),3);
error2 = std(subAvg.Fig2.IRFx2_IRF(:,2,:),0,3)/sqrt(numel(NE_order));
...
colorbar off;
title '';
exportgraphics(f, fullfile(fig_savePath,'Figure2_H.jpg'),'Resolution',300,'BackgroundColor',[1 1 1]);
...
colorbar off;
title '';
exportgraphics(f, fullfile(fig_savePath,'Figure2_I.jpg'),'Resolution',300,'BackgroundColor',[1 1 1]);
...
colorbar off;
title '';
exportgraphics(f, fullfile(fig_savePath,'Figure2_J2.jpg'),'Resolution',300,'BackgroundColor',[1 1 1]);
...
f = figure;
[dataMean, dataSEM] = f_plotBar(barData,colors=[repmat(c_Yellow,3,1);repmat(c_darkCyan,2,1);repmat(c_pupil,2,1)],ylabel='r',title='Model Performance Comparison')
ylim([0, 1]);
...
T = table({order.Mouse}',T(:,1),T(:,2),T(:,3),T(:,4),T(:,5),T(:,6),T(:,7), ...
