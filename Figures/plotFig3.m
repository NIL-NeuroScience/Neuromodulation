...
% 
% Plots figure panels for Figure 3. Must run MAIN_plotFigures.m first.
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
subAvg.Fig3.lowNE_FC_Ca = NaN(12, 12, M_NE);
subAvg.Fig3.lowNE_FC_HbT = NaN(12, 12, M_NE);
subAvg.Fig3.highNE_FC_Ca = NaN(12, 12, M_NE);
subAvg.Fig3.highNE_FC_HbT = NaN(12, 12, M_NE);
subAvg.Fig3.FC_Ca_HbT_vs_GRAB = NaN(M_NE, 1);
subAvg.Fig3.FC_Ca_vs_GRAB = NaN(12, 12, M_NE);
subAvg.Fig3.FC_HbT_vs_GRAB = NaN(12, 12, M_NE);

for i = 1 : M_NE
    subAvg.Fig3.lowNE_FC_Ca(:, :, i) = mean(cat(3, ...
        Fig3.lowNE_Ca{NE_order(i).Runs}), 3, 'omitnan');
    subAvg.Fig3.lowNE_FC_HbT(:, :, i) = mean(cat(3, ...
        Fig3.lowNE_HbT{NE_order(i).Runs}), 3, 'omitnan');
    subAvg.Fig3.highNE_FC_Ca(:, :, i) = mean(cat(3, ...
        Fig3.highNE_Ca{NE_order(i).Runs}), 3, 'omitnan');
    subAvg.Fig3.highNE_FC_HbT(:, :, i) = mean(cat(3, ...
        Fig3.highNE_HbT{NE_order(i).Runs}), 3, 'omitnan');
    subAvg.Fig3.FC_Ca_HbT_vs_GRAB(i) = mean(cat(1, ...
        Fig3.FC_Ca_HbT_vs_GRAB{NE_order(i).Runs}));
    subAvg.Fig3.FC_Ca_vs_GRAB(:, :, i) = mean(cat(3, ...
        Fig3.FC_Ca_vs_GRAB{NE_order(i).Runs}), 3);
    subAvg.Fig3.FC_HbT_vs_GRAB(:, :, i) = mean(cat(3, ...
        Fig3.FC_HbT_vs_GRAB{NE_order(i).Runs}), 3);
end
...
T = table(t(idx)', Ca(idx, 5), NE(idx, 5), HbT(idx, 5), Ca(idx, 2), ...
    NE(idx, 2), HbT(idx, 2), Pupil(idx), Whisking(idx), ...
    Accelerometer(idx), VariableNames = {'Time', 'Ca_SSpll', ...
    'NE_SSpll', 'HbT_SSpll', 'Ca_MOs', 'NE_MOs', 'HbT_MOs', 'Pupil', ...
...
[dataMean, dataSEM] = f_plotBar(barData, ...
    colors = [c_Ca; c_HbT; c_darkCyan], ...
    legend = {'Ca^2^+', 'HbT', 'Ca^2^+ vs. HbT'}, ...
    ylabel = 'r');
...
f = figure;
f_plotFC(mean(subAvg.Fig3.lowNE_FC_Ca, 3), 1, ...
    cmp = cmpvir, ...
    clim = [0, 1], ...
    clabel = 'r');
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'Figure3_G1.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);
writetable(table(mean(subAvg.Fig3.lowNE_FC_Ca, 3)), ...
    fullfile(fig_savePath, 'Figure3_G1.csv'));

f = figure;
f_plotFC(mean(subAvg.Fig3.highNE_FC_Ca, 3), 1, ...
    cmp = cmpvir, ...
    clim = [0, 1], ...
    clabel = 'r');
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'Figure3_G2.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);
writetable(table(mean(subAvg.Fig3.highNE_FC_Ca, 3)), ...
    fullfile(fig_savePath, 'Figure3_G2.csv'));
...
f = figure;
f_plotFC(mean(subAvg.Fig3.highNE_FC_Ca, 3) - ...
    mean(subAvg.Fig3.lowNE_FC_Ca, 3), 0, ...
    cmp = cmpbbr, ...
    clim = 0.25 * [-1, 1]);
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'Figure3_G3.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);
...
writetable(table(mean(subAvg.Fig3.highNE_FC_Ca, 3) - ...
    mean(subAvg.Fig3.lowNE_FC_Ca, 3)), ...
    fullfile(fig_savePath, 'Figure3_G3.csv'));
writetable(table(reshape(p, 12, 12)), ...
    fullfile(fig_savePath, 'Figure3_G3_P.csv'));

%% Fig 3H

f = figure;
f_plotFC(mean(subAvg.Fig3.lowNE_FC_HbT, 3), 1, ...
    cmp = cmpvir, ...
    clim = [0, 1], ...
    clabel = 'r');
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'Figure3_H1.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);
writetable(table(mean(subAvg.Fig3.lowNE_FC_HbT, 3)), ...
    fullfile(fig_savePath, 'Figure3_H1.csv'));

f = figure;
f_plotFC(mean(subAvg.Fig3.highNE_FC_HbT, 3), 1, ...
    cmp = cmpvir, ...
    clim = [0, 1], ...
    clabel = 'r');
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'Figure3_H2.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);
writetable(table(mean(subAvg.Fig3.highNE_FC_HbT, 3)), ...
    fullfile(fig_savePath, 'Figure3_H2.csv'));
...
f = figure;
f_plotFC(mean(subAvg.Fig3.highNE_FC_HbT, 3) - ...
    mean(subAvg.Fig3.lowNE_FC_HbT, 3), 0, ...
    cmp = cmpbbr, ...
    clim = 0.1 * [-1, 1]);
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'Figure3_H3.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);
...
writetable(table(mean(subAvg.Fig3.highNE_FC_Ca, 3) - ...
    mean(subAvg.Fig3.lowNE_FC_Ca, 3)), ...
    fullfile(fig_savePath, 'Figure3_H3.csv'));
writetable(table(reshape(p, 12, 12)), ...
    fullfile(fig_savePath, 'Figure3_H3_P.csv'));
...
for i = 1 : 3
    ...
    exportgraphics(f, fullfile(fig_savePath, ...
        sprintf('Figure3_I%01i_low.jpg', i)), ...
        Resolution = 300, ...
        BackgroundColor = [1, 1, 1]);
    ...
    exportgraphics(f, fullfile(fig_savePath, ...
        sprintf('Figure3_I%01i_high.jpg', i)), ...
        Resolution = 300, ...
        BackgroundColor = [1, 1, 1]);
    ...
    exportgraphics(f, fullfile(fig_savePath, ...
        sprintf('Figure3_I%01i_diff.jpg', i)), ...
        Resolution = 300, ...
        BackgroundColor = [1, 1, 1]);
end
...
for i = 1 : 3
    ...
    exportgraphics(f, fullfile(fig_savePath, ...
        sprintf('Figure3_J%01i_low.jpg', i)), ...
        Resolution = 300, ...
        BackgroundColor = [1, 1, 1]);

    f = figure;
    f_plotAllenMap(allenHbTHigh(i, :), ...
        cmp = cmpvir, ...
        mask = plotBM, ...
        clim = [0, 1]);
    f_plotAllenRegion(regions(i), 2, ...
    ...
    exportgraphics(f, fullfile(fig_savePath, ...
        sprintf('Figure3_J%01i_high.jpg', i)), ...
        Resolution = 300, ...
        BackgroundColor = [1, 1, 1]);

    f = figure;
    f_plotAllenMap(allenHbTHigh(i, :) - allenHbTLow(i, :), ...
        cmp = cmpbbr, ...
        mask = plotBM, ...
        clim = 0.1 * [-1, 1]);
    f_plotAllenRegion(regions(i), 2, ...
    ...
    exportgraphics(f, fullfile(fig_savePath, ...
        sprintf('Figure3_J%01i_diff.jpg', i)), ...
        Resolution = 300, ...
        BackgroundColor = [1, 1, 1]);
end