...
% 
% Plots figure panels for Extended Data Figure 10. Must run 
...
Mice_E10 = fieldnames(metadata);
M_E10 = numel(Mice_E10);

subAvg.FigE10.XC_gfp_HD_HbT_low = NaN(201, M_E10);
subAvg.FigE10.R_gfp_HD_HbT_low = cell(3, 1);

for i = 1 : M_E10
    subAvg.FigE10.XC_gfp_HD_HbT_low(:, i) = mean( ...
        [metadata.(Mice_E10{i}).XC_gfp_HD_HbT_low], 2);
    subAvg.FigE10.R_gfp_HD_HbT_low{i} = mean( ...
        cat(3, metadata.(Mice_E10{i}).R_gfp_HD_HbT_low), 3);
end
...
f = figure;
f_plotMap(runData.int_Ca, ...
    clim = [0, 30000], ...
    cmp = cmpinf)
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'ExtDataFig10_C1.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);

for i = 1 : 2
    plot(mask{i}(:, 2), mask{i}(:, 1), '-', ...
        color = 0.7 * [1, 1, 1], ...
        lineWidth = 2);
end
...
f = figure;
f_plotMap(runData.std_Ca, ...
    clim = [0, 5], ...
    cmp = cmpinf);
colorbar off;

exportgraphics(f, fullfile(fig_savePath, 'ExtDataFig10_C2.jpg'), ...
    Resolution = 300, ...
    BackgroundColor = [1, 1, 1]);
...
f = figure;
hold on;
scatter(int_Ca(1 : skip : end), std_Ca(1 : skip : end), 50, 'filled', ...
    MarkerFaceAlpha = 0.1, ...
...
dataMutant = load(fullfile(f_path, ...
    'Figures/results/HD/mutant_data_B6_248_250204.mat'));
...
T = table(t(idx)', dataMutant.HbT(idx, region), ...
    dataMutant.gfp(idx, region), dataMutant.gfp_HD(idx, region), ...
...
f = figure;
plot(t, xcorr(data2P.dil, data2P.NE_sig, 75, 'normalized'), ...
    color = 0.7 * [1, 1, 1]);
ylim(0.5 * [-1, 1]);
xlim([-5, 5]);
box off;
...
T = table(t', meanSig1, meanSig2, meanSig3, error1, error2, error3, ...
    VariableNames = {'Time', 'mean_NE', 'mean_ACh', 'mean_NEmut', ...
    'SEM_NE', 'SEM_ACh', 'SEM_NEmut'});
