...
files = struct;
files.metadata = fullfile(parentDir,'analysis_final');
...
for nwbI = 1:numel(nwb_list)
    ...
    [Fig1.IRFx1_inv.perf,Fig1.IRFx1_inv.IRF,Fig1.IRFx1_inv.params,tmpCorr] = f_1xIRF(HbT_low,rfp_HD,win*fs,brain_mask,ds=4,corrWin=corrWin*fs,maxThreads=numThreads);
    ...
    [Fig1.IRFx1_SSp.perf,Fig1.IRFx1_SSp.IRF,Fig1.IRFx1_SSp.params,tmpCorr] = f_1xIRF(HbT_low,rfp_HD,win*fs,SSp.*brain_mask,ds=4,corrWin=corrWin*fs,maxThreads=numThreads);
    Fig1.IRFx1_SSp.perf_dt = squeeze(mean(tmpCorr.*permute(allen_masks,[1 2 4 3]),[1 2],'omitnan'));
    
    % estimate variant IRF model
    [Fig1.IRFx1_var.perf,Fig1.IRFx1_var.IRF,Fig1.IRFx1_var.params,tmpCorr] = f_1xIRF_varWeights(HbT_low,rfp_HD,win*fs,brain_mask,ds=ds,corrWin=corrWin*fs,maxThreads=numThreads);
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        Fig2.LR = struct;
        [Fig2.LR.perf,Fig2.LR.params,Fig2.LR_Ca,Fig2.LR_NE] = f_LR_varWeights(HbT_low,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=ds,method='fft',maxThreads=numThreads);
        ...
        Fig2.IRFx2 = struct;
        [Fig2.IRFx2.perf,Fig2.IRFx2.IRF,Fig2.IRFx2.params,Fig2.IRFx2_Ca,Fig2.IRFx2_NE] = f_2alphaDeconvolve(HbT_low,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=ds,method='fft',maxThreads=numThreads);
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        [Hb_model.HbO.LR.perf,Hb_model.HbO.LR.params] = f_LR_varWeights(HbO_low,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=ds,method='fft',maxThreads=numThreads);
        
        [Hb_model.HbO.IRFx2.perf,Hb_model.HbO.IRFx2.IRF,Hb_model.HbO.IRFx2.params] = f_2alphaDeconvolve(HbO_low,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=ds,method='fft',maxThreads=numThreads);
        ...
        [Hb_model.Hb.LR.perf,Hb_model.Hb.LR.params] = f_LR_varWeights(Hb_low,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=32,method='fft',maxThreads=numThreads);
        
        [Hb_model.Hb.IRFx2.perf,Hb_model.Hb.IRFx2.IRF,Hb_model.Hb.IRFx2.params] = f_2alphaDeconvolve(Hb_low,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=ds,method='fft',maxThreads=numThreads);
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        % estimate linear regression model
        [unfiltered.LR.perf,unfiltered.LR.params] = f_LR_varWeights(HbT,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=32,method='fft',maxThreads=numThreads);
        
        % estimate double IRF model - varying weights
        [unfiltered.IRFx2.perf,unfiltered.IRFx2.IRF,unfiltered.IRFx2.params] = f_2alphaDeconvolve(HbT,rfp_HD,globalNE.*ones(size(rfp_HD,[1,2])),win*fs,brain_mask,ds=ds,method='fft',maxThreads=numThreads);
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        for i = 1:3
            % estimate linear regression model
            [shuffled.LR(i).perf,shuffled.LR(i).params] = f_LR_varWeights(HbT_low,rfp_HD,circshift(globalNE.*ones(size(rfp_HD,[1,2])),i*shift,3),win*fs,brain_mask,ds=32,method='fft',maxThreads=numThreads);
            
            % estimate double IRF model - varying weights
            [shuffled.IRFx2(i).perf,shuffled.IRFx2(i).IRF,shuffled.IRFx2(i).params] = f_2alphaDeconvolve(HbT_low,rfp_HD,circshift(globalNE.*ones(size(rfp_HD,[1,2])),i*shift,3),win*fs,brain_mask,ds=ds,method='fft',maxThreads=numThreads);
