...
for nwbI = 1 : numel(nwb_list)
    ...
    savePath = fullfile(files.metadata, savePath);

    %%
    fprintf(['\nAnalyzing ', nwb_list(nwbI).Path, '...']);

    % read and load data from nwb file
    nwb = nwbRead(nwb_list(nwbI).Path);
    [~, gfp, rfp_HD, gfp_HD, Hb, HbO, HbT, Whisking, Pupil, ...
        Accelerometer, brain_mask, vessel_mask, allen_masks, fs, ...
        mouseInfo, sessionInfo] = f_extractNWB(nwb);
    ...
    [Fig1.IRFx1_inv.perf, Fig1.IRFx1_inv.IRF, Fig1.IRFx1_inv.params, ...
        tmpCorr] = f_1xIRF(HbT_low, rfp_HD, win * fs, brain_mask, ...
        ds = 4, ...
        corrWin = corrWin * fs, ...
        maxThreads = numThreads);
    Fig1.IRFx1_inv.perf_dt = f_parcellate(tmpCorr, allen_masks);
    
    % estimate medial SSp single IRF model 
    SSp = sum(allen_masks(:, :, [4, 5]), 3, 'omitnan');
    SSp(SSp == 0) = NaN;
    
    [Fig1.IRFx1_SSp.perf, Fig1.IRFx1_SSp.IRF, Fig1.IRFx1_SSp.params, ...
        tmpCorr] = f_1xIRF(HbT_low, rfp_HD, ...
        win * fs, SSp .* brain_mask, ...
        ds = 4, ...
        corrWin = corrWin * fs, ...
        maxThreads = numThreads);
    Fig1.IRFx1_SSp.perf_dt = f_parcellate(tmpCorr, allen_masks);
    
    % estimate variant IRF model
    [Fig1.IRFx1_var.perf, Fig1.IRFx1_var.IRF, Fig1.IRFx1_var.params, ...
        tmpCorr] = f_1xIRF_varWeights(HbT_low, rfp_HD, win * fs, ...
        brain_mask, ...
        ds = ds, ...
        corrWin = corrWin * fs, ...
        maxThreads = numThreads);
    Fig1.IRFx1_var.perf_dt = f_parcellate(tmpCorr, allen_masks);
    Fig1.IRFx1_var.IRF_allen = f_parcellate(Fig1.IRFx1_var.IRF, ...
        allen_masks);
    
    % apply moving window to GRAB and pupil
    N = numel(Pupil);
    win_idx = corrWin(1) * fs / 2 : corrWin(2) * fs : ...
        N - corrWin(1) * fs / 2;

    Fig1.GRAB = f_parcellate(gfp_HD, allen_masks .* vessel_mask);
    Fig1.GRAB = movmean(Fig1.GRAB, corrWin(2) * fs);
    Fig1.GRAB = Fig1.GRAB(win_idx, :);
    
    Fig1.GRAB_global = f_parcellate(gfp_HD, brain_mask .* vessel_mask);
    Fig1.GRAB_global_ds = movmean(Fig1.GRAB_global, corrWin(2) * fs);
    Fig1.GRAB_global_ds = Fig1.GRAB_global_ds(win_idx, :);
    
    Fig1.Pupil = movmean(Pupil, corrWin(2) * fs);
    Fig1.Pupil = Fig1.Pupil(win_idx);
    
    % calculate correlation coefficient - GRAB and pupil vs. SSp IRF 
    % performance
    Fig1.SSp_perf_vs_pupil = corrcoef([Fig1.Pupil, ...
        Fig1.IRFx1_SSp.perf_dt]);
    Fig1.SSp_perf_vs_pupil = Fig1.SSp_perf_vs_pupil(2 : 13)';
    
    Fig1.SSp_perf_vs_GRAB = f_corr(Fig1.GRAB, Fig1.IRFx1_SSp.perf_dt, 1)';
    Fig1.SSp_perf_vs_GRAB_global = f_corr(Fig1.GRAB_global_ds, ...
        Fig1.IRFx1_SSp.perf_dt, 1)';
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        Fig2.NE = f_parcellate(gfp_HD ./ std(gfp_HD, 0, 3), ...
            brain_mask .* vessel_mask);
        globalNE = permute(Fig2.NE, [2, 3, 1]);
        Fig2.Ca_norm = f_parcellate(rfp_HD ./ std(rfp_HD, 0, 3), ...
            allen_masks .* vessel_mask);
        Fig2.HbT_norm = f_parcellate(HbT_low ./ std(HbT_low, 0, 3), ...
            allen_masks);

        % estimate linear regression model
        Fig2.LR = struct;
        [Fig2.LR.perf, Fig2.LR.params, Fig2.LR_Ca, Fig2.LR_NE] = ...
            f_LR_varWeights(HbT_low, rfp_HD, globalNE .* ...
            ones(size(rfp_HD, [1, 2])), win * fs, brain_mask, ...
            ds = ds, ...
            method = 'direct', ...
            maxThreads = numThreads);
        ...
        Fig2.IRFx2 = struct;
        [Fig2.IRFx2.perf, Fig2.IRFx2.IRF, Fig2.IRFx2.params, ...
            Fig2.IRFx2_Ca, Fig2.IRFx2_NE] = f_2alphaDeconvolve( ...
            HbT_low, rfp_HD, globalNE .* ones(size(rfp_HD, [1, 2])), ...
            win * fs, brain_mask, ...
            ds = ds, ...
            method = 'direct', ...
            maxThreads = numThreads);
        Fig2.IRFx2.params.A = Fig2.IRFx2.params.A * ...
            sum(Fig2.IRFx2.IRF(:, 1));
        Fig2.IRFx2.params.B = Fig2.IRFx2.params.B * ...
            sum(Fig2.IRFx2.IRF(:, 2));
        Fig2.IRFx2_Ca = f_parcellate(Fig2.IRFx2_Ca, ...
            brain_mask .* allen_masks);
        Fig2.IRFx2_NE = f_parcellate(Fig2.IRFx2_NE, ...
            brain_mask .* allen_masks);
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        Fig3.GRAB = Fig3.GRAB(win(1) * fs / 2 : win(2) * fs : ...
            size(Fig3.GRAB, 1) - win(1) * fs / 2);
        ...
        Fig3.FC.Ca_vs_HbT = f_corr(tmpCa(trilIdx, :), ...
            tmpHbT(trilIdx, :), 1)';
        
        % calculate correlation measures - FC Ca, FC HbT, and FC Ca/HbT vs. GRAB
        
        Fig3.r.FC_Ca_vs_GRAB = f_corr(Fig3.FC.Ca, ...
            permute(Fig3.GRAB, [2, 3, 1]), 3);
        Fig3.r.FC_HbT_vs_GRAB = f_corr(Fig3.FC.HbT, ...
            permute(Fig3.GRAB, [2, 3, 1]), 3);
    ...
    GRAB_FC.GRAB_norm = f_parcellate(gfp_HD ./ std(gfp_HD, 0, 3), ...
        allen_masks .* vessel_mask);
    ...
    % spectrum
    [Behavior.SPC.rfp_HD, Behavior.SPC.fr] = f_hemSpectra(rfp_HD,fs, ...
        [0, round(fs / 2)], comb_mask, Behavior.tapers);
    [Behavior.SPC.gfp_HD] = f_hemSpectra(gfp_HD, fs, ...
        [0, round(fs / 2)], comb_mask, Behavior.tapers);
    [Behavior.SPC.gfp] = f_hemSpectra(gfp, fs, [0, round(fs / 2)], ...
        comb_mask, Behavior.tapers);
    [Behavior.SPC.HbT] = f_hemSpectra(HbT, fs, [0, round(fs / 2)], ...
        brain_mask, Behavior.tapers);
    
    % coherence
    [Behavior.COH.rfp_HD_HbT, Behavior.PHI.rfp_HD_HbT] = ...
        f_hemCoherence(rfp_HD, HbT, fs, comb_mask, Behavior.tapers);
    [Behavior.COH.gfp_HD_HbT, Behavior.PHI.gfp_HD_HbT] = ...
        f_hemCoherence(gfp_HD, HbT, fs, comb_mask, Behavior.tapers);
    [Behavior.COH.gfp_HbT, Behavior.PHI.gfp_HbT] = f_hemCoherence( ...
        gfp, HbT, fs, comb_mask, Behavior.tapers);
    [Behavior.COH.rfp_HD_gfp_HD, Behavior.PHI.rfp_HD_gfp_HD] = ...
        f_hemCoherence(rfp_HD, gfp_HD, fs, comb_mask, Behavior.tapers);
    ...
    Behavior.R.rfp_HD_low_gfp_HD_low = f_corr( ...
        f_bpf(rfp_HD, [0, 0.5], fs, 3), f_bpf(gfp_HD, [0, 0.5], fs, 3), 3);
    Behavior.R.rfp_HD_low_HbT_low = f_corr( ...
        f_bpf(rfp_HD, [0, 0.5], fs, 3), HbT_low, 3);
    Behavior.R.gfp_HD_low_HbT_low = f_corr( ...
        f_bpf(gfp_HD, [0, 0.5], fs, 3), HbT_low, 3);
    Behavior.R.gfp_low_HbT_low = f_corr( ...
        f_bpf(gfp, [0, 0.5], fs, 3), HbT_low, 3);
    Behavior.signals = [f_parcellate(rfp_HD, comb_mask), ...
        f_parcellate(gfp_HD, comb_mask), ...
        f_parcellate(HbT_low, brain_mask), ...
        Pupil, Whisking, Accelerometer];
    Behavior.R.signals = corrcoef(Behavior.signals);
    
    % estimate Ca x IRF = NE
    if string(mouseInfo.GRAB) == "GRAB_NE"
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        [Hb_model.HbO.LR.perf, Hb_model.HbO.LR.params] = ...
            f_LR_varWeights(HbO_low, rfp_HD, globalNE .* ...
            ones(size(rfp_HD, [1, 2])), win * fs, brain_mask, ...
            ds = ds, ...
            method = 'direct', ...
            maxThreads = numThreads);
        
        [Hb_model.HbO.IRFx2.perf, Hb_model.HbO.IRFx2.IRF, ...
            Hb_model.HbO.IRFx2.params] = f_2alphaDeconvolve(HbO_low, ...
            rfp_HD, globalNE .* ones(size(rfp_HD, [1, 2])), win * fs, ...
            brain_mask, ...
            ds = ds, ...
            method = 'direct', ...
            maxThreads = numThreads);
        Hb_model.HbO.IRFx2.params.A = Hb_model.HbO.IRFx2.params.A * ...
            sum(Hb_model.HbO.IRFx2.IRF(:, 1));
        Hb_model.HbO.IRFx2.params.B = Hb_model.HbO.IRFx2.params.B * ...
            sum(Hb_model.HbO.IRFx2.IRF(:, 2));
        
        % Hb
        Hb_low = f_bpf(Hb, [0, 0.5], fs, 3);
        [Hb_model.Hb.LR.perf, Hb_model.Hb.LR.params] = f_LR_varWeights( ...
            Hb_low, rfp_HD, globalNE .* ones(size(rfp_HD, [1, 2])), ...
            win * fs, brain_mask, ...
            ds = 32, ...
            method = 'direct', ...
            maxThreads = numThreads);
        
        [Hb_model.Hb.IRFx2.perf, Hb_model.Hb.IRFx2.IRF, ...
            Hb_model.Hb.IRFx2.params] = f_2alphaDeconvolve(Hb_low, ...
            rfp_HD, globalNE .* ones(size(rfp_HD, [1, 2])), ...
            win * fs, brain_mask, ...
            ds = ds, ...
            method = 'direct', ...
            maxThreads = numThreads);
        Hb_model.Hb.IRFx2.params.A = Hb_model.Hb.IRFx2.params.A * ...
            sum(Hb_model.Hb.IRFx2.IRF(:, 1));
        Hb_model.Hb.IRFx2.params.B = Hb_model.Hb.IRFx2.params.B * ...
            sum(Hb_model.Hb.IRFx2.IRF(:, 2));
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        % estimate linear regression model
        [unfiltered.LR.perf, unfiltered.LR.params] = f_LR_varWeights( ...
            HbT, rfp_HD, globalNE .* ones(size(rfp_HD, [1, 2])), ...
            win * fs, brain_mask, ...
            ds = 32, ...
            method = 'direct', ...
            maxThreads = numThreads);
        ...
        % estimate double IRF model - varying weights
        [unfiltered.IRFx2.perf, unfiltered.IRFx2.IRF, ...
            unfiltered.IRFx2.params] = f_2alphaDeconvolve(HbT, rfp_HD, ...
            globalNE .* ones(size(rfp_HD, [1, 2])), win * fs, ...
            brain_mask, ...
            ds = ds, ...
            method = 'direct', ...
            maxThreads = numThreads);
        unfiltered.IRFx2.params.A = unfiltered.IRFx2.params.A * ...
            sum(unfiltered.IRFx2.IRF(:, 1));
        unfiltered.IRFx2.params.B = unfiltered.IRFx2.params.B * ...
            sum(unfiltered.IRFx2.IRF(:, 2));
    end
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        for i = 1 : 3
            % estimate linear regression model
            [shuffled.LR(i).perf, shuffled.LR(i).params] = ...
                f_LR_varWeights(HbT_low, rfp_HD, circshift(globalNE .* ...
                ones(size(rfp_HD, [1, 2])), i * shift, 3), win * fs, ...
                brain_mask, ...
                ds = 32, ...
                method = 'direct', ...
                maxThreads = numThreads);
            ...
            % estimate double IRF model - varying weights
            [shuffled.IRFx2(i).perf, shuffled.IRFx2(i).IRF, ...
                shuffled.IRFx2(i).params] = f_2alphaDeconvolve(HbT_low, ...
                rfp_HD, circshift(globalNE .* ones(size(rfp_HD, ...
                [1, 2])), i * shift, 3), win * fs, brain_mask, ...
                ds = ds, ...
                method = 'direct', ...
                maxThreads = numThreads);
            shuffled.IRFx2(i).params.A = shuffled.IRFx2(i).params.A * ...
                sum(shuffled.IRFx2(i).IRF(:, 1));
            shuffled.IRFx2(i).params.B = shuffled.IRFx2(i).params.B * ...
                sum(shuffled.IRFx2(i).IRF(:, 2));
        end
    
        % summarize shuffling
        shuffled.summary.LR.perf = mean(cat(3, shuffled.LR(:).perf), 3);
        shuffled.summary.IRFx2.perf = mean(cat(3, ...
            shuffled.IRFx2(:).perf),3);
        shuffled.summary.IRFx2.IRF = mean(cat(3, ...
            shuffled.IRFx2(:).IRF), 3);
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        NE_reg.HbT_reg_global = Fig3.HbT - NE_reg.HbT_reg_global_LR .* ...
            squeeze(globalNE);
        ...
        NE_reg.highNE_FC.HbT_reg = mean( ...
            NE_reg.FC.HbT_reg(:, :, highNE), 3);

        NE_reg.FC.HbT_reg_global = f_funConGram( ...
            NE_reg.HbT_reg_global, win * fs);
        NE_reg.lowNE_FC.HbT_reg_global = mean( ...
            NE_reg.FC.HbT_reg_global(:, :, lowNE), 3);
        NE_reg.highNE_FC.HbT_reg_global = mean( ...
            NE_reg.FC.HbT_reg_global(:, :, highNE), 3);
        
        NE_reg.FC_r.Ca_HbT = f_corr(Fig3.FC.Ca, Fig3.FC.HbT, 3);
        NE_reg.FC_r.Ca_HbT_reg = f_corr(Fig3.FC.Ca, NE_reg.FC.HbT_reg, 3);
        NE_reg.FC_r.Ca_HbT_reg_global = f_corr( ...
            Fig3.FC.Ca, NE_reg.FC.HbT_reg_global, 3);
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        for i = 1 : 3
            ...
            FC_fr.(labels{i}).Ca = f_parcellate( ...
                tmpFilt ./ std(tmpFilt, 0, 3), vessel_mask .* allen_masks);
            FC_fr.(labels{i}).FC.Ca = f_funConGram( ...
                FC_fr.(labels{i}).Ca, win * fs);
        
            FC_fr.(labels{i}).lowNE_FC.Ca = mean( ...
                FC_fr.(labels{i}).FC.Ca(:, :, lowNE), 3);
            FC_fr.(labels{i}).highNE_FC.Ca = mean( ...
                FC_fr.(labels{i}).FC.Ca(:, :, highNE), 3);
        
            if i ~= 3
                tmpFilt = f_bpf(HbT, fr_range(i, :), fs, 3);
                FC_fr.(labels{i}).HbT = f_parcellate( ...
                    tmpFilt ./ std(tmpFilt, 0, 3), vessel_mask .* allen_masks);
                FC_fr.(labels{i}).FC.HbT = f_funConGram( ...
                    FC_fr.(labels{i}).HbT, win * fs);
        
                FC_fr.(labels{i}).lowNE_FC.HbT = mean( ...
                    FC_fr.(labels{i}).FC.HbT(:, :, lowNE), 3);
                FC_fr.(labels{i}).highNE_FC.HbT = mean( ...
                    FC_fr.(labels{i}).FC.HbT(:, :, highNE), 3);
            end
        end
    end
    ...
    if string(mouseInfo.GRAB) == "GRAB_NE"
        ...
        spectra.Ca = f_parcellate(rfp_HD, vessel_mask .* allen_masks);
        spectra.HbT = f_parcellate(HbT, allen_masks);
        spectra.NE = f_parcellate(gfp_HD, vessel_mask .* allen_masks);
        
        [spectra.SPG.Ca, spectra.SPG.t, spectra.SPG.f] = f_morlet( ...
            spectra.Ca, fs, [0.01, fs / 2], 300);
        ...
        spectra.low_NE.Ca = squeeze(mean( ...
            spectra.SPG.Ca(low_idx, :, :), 1));
        spectra.low_NE.HbT = squeeze(mean( ...
            spectra.SPG.HbT(low_idx, :, :), 1));
    
        spectra.high_NE.Ca = squeeze(mean( ...
            spectra.SPG.Ca(high_idx, :, :), 1));
        spectra.high_NE.HbT = squeeze(mean( ...
            spectra.SPG.HbT(high_idx, :, :), 1));
    ...
    if saveMetadata
        ...
        metadata.upd = '25-11-10';
        metadata.GRAB = mouseInfo.GRAB;
        
        if string(mouseInfo.GRAB) == "GRAB_NE"
            save(savePath, 'metadata', 'Fig1', 'GRAB_FC', 'Fig2', ...
                'Fig3', 'Behavior', 'Hb_model', 'unfiltered', ...
                'shuffled', 'NE_reg', 'FC_fr', 'spectra', '-v7.3');
        else
            save(savePath, 'metadata', 'Fig1', 'GRAB_FC', 'Behavior', ...
                '-v7.3');
        end
        fprintf('\n\tDone!\n');
    end
